<template>
            ...
            <v-table class="mTable" hover>
                ...
                <tbody>
                    <tr v-for="operation in processList" :key="operation">
                        ...
                        <td :data-title="$t('P')">{{ operation.pub_id }}</td>
                        <td :data-title="$t('e')">{{ operation.pub_name }}</td>
                        <td :data-title="$t('t')">{{ operation.city }}</td>
                        <td :data-title="$t('e')">{{ operation.state }}</td>
                        <td :data-title="$t('r')">{{ operation.country }}</td>
    ...
    <v-dialog v-model="isNewShow" max-width="600px">
        <v-form ref="form">
            <v-card>
                <v-card-title>{{ $t('newPub') }}</v-card-title>
                <v-card-text>
                    <v-text-field v-model="insertPubInfo.pub_id" label="ID "></v-text-field>
                    <v-text-field v-model="insertPubInfo.pub_name" label="pub_name"   ></v-text-field>
                    <v-text-field v-model="insertPubInfo.city" label="city"  ></v-text-field>
                    <v-text-field v-model="insertPubInfo.state" label="state" ></v-text-field>
                    <v-text-field v-model="insertPubInfo.country" label="country"></v-text-field>

                </v-card-text>
                <v-card-actions>
                    <v-btn @click="insertPub()">{{ $t('save') }}</v-btn>
                </v-card-actions>
            </v-card>
        </v-form>
    </v-dialog>
...
export default {
    ...
    // },
        ...
        insertPub() {
            sendPost('WeatherForecast/publishID', this.insertPubInfo, {}, () => {
                this.getOperationList(1);
                this.getProcessList();
                alert('成功');
                setTimeout(() => {
  
            window.location.href = '/PubIdtest';
        }, 0);
            }, (err) => {
                console.error(err);
            });
        ...
        getProcessList() {
            ...
            sendGet('WeatherForecast/PubSelect', {}, { pub_name: this.search.value }, (res) => {
